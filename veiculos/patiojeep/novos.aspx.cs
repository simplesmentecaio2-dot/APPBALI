...
(
    SELECT
        'NOVO' COLLATE DATABASE_DEFAULT AS tipo,
        CONVERT(varchar(50), p.ve_nr) COLLATE DATABASE_DEFAULT AS ve_nr,
        CONVERT(varchar(50), p.ve_nr) COLLATE DATABASE_DEFAULT AS codigo_origem,
        SUBSTRING(COALESCE(NULLIF(v.Veiculo_Descricao, ''), 'Veiculo'), 1, 90) COLLATE DATABASE_DEFAULT AS ve_ds,
        ISNULL(v.Veiculo_Chassi, '') COLLATE DATABASE_DEFAULT AS ve_chassi,
        ISNULL(v.Veiculo_ChassiSerie, '') COLLATE DATABASE_DEFAULT AS ve_serie,
        ISNULL(v.Veiculo_Placa, '') COLLATE DATABASE_DEFAULT AS ve_placa,
        ISNULL(CONVERT(varchar(40), v.Veiculo_NroRenavam), '') COLLATE DATABASE_DEFAULT AS ve_renavam,
        ISNULL(c.Cor_Descricao, '') COLLATE DATABASE_DEFAULT AS cor_ds,
        COALESCE(NULLIF(p.loja_atual_id, 0), p.loja_id) AS loja_atual_id,
        COALESCE(lj.ds, 'Sem loja') COLLATE DATABASE_DEFAULT AS loja_atual,
        ISNULL(p.fun_cad, '') COLLATE DATABASE_DEFAULT AS fun_cad,
        p.dt_cad,
    ...
    SELECT
        'SEMINOVO' COLLATE DATABASE_DEFAULT AS tipo,
        CONVERT(varchar(50), s.id) COLLATE DATABASE_DEFAULT AS ve_nr,
        CONVERT(varchar(50), s.ve_nr) COLLATE DATABASE_DEFAULT AS codigo_origem,
        SUBSTRING(COALESCE(NULLIF(s.ve_ds, ''), 'Seminovo'), 1, 90) COLLATE DATABASE_DEFAULT AS ve_ds,
        ISNULL(s.ve_chassi, '') COLLATE DATABASE_DEFAULT AS ve_chassi,
        RIGHT(ISNULL(s.ve_chassi, ''), 7) COLLATE DATABASE_DEFAULT AS ve_serie,
        ISNULL(s.ve_placa, '') COLLATE DATABASE_DEFAULT AS ve_placa,
        ISNULL(s.ve_renavam, '') COLLATE DATABASE_DEFAULT AS ve_renavam,
        ISNULL(s.cor_ds, '') COLLATE DATABASE_DEFAULT AS cor_ds,
        COALESCE(NULLIF(s.loja_atual_id, 0), s.loja_id) AS loja_atual_id,
        COALESCE(lj.ds, 'Sem loja') COLLATE DATABASE_DEFAULT AS loja_atual,
        ISNULL(s.fun_cad, '') COLLATE DATABASE_DEFAULT AS fun_cad,
        s.dt_cad,
